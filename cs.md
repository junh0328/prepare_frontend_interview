...
📌 **관련 주제**: [HTTP](#http), [운영체제](#운영체제), [프로세스와 스레드](#프로세스와-스레드), [실시간 통신](#실시간-통신), [아키텍처 - 시스템/인프라 아키텍처](./architecture.md#시스템인프라-아키텍처)
...
📌 **관련 주제**: [네트워크](#네트워크), [CORS](#cors), [쿠키 세션](#쿠키-세션), [실시간 통신](#실시간-통신)
...
누군가가 악의적으로 이 요청을 가로채면 아이디와 비밀번호가 그대로 노출됩니다.

하지만 HTTPS를 사용할 경우, 서버만 알아볼 수 있도록 정보를 **대칭키(세션 키)**로 암호화해서 보냅니다.
...
SSL(Secure Sockets Layer)은 SSL 1.0부터 시작해서 SSL 2.0, SSL 3.0, TLS 1.0, TLS 1.1, TLS 1.2, TLS 1.3까지 버전이 올라가며 마지막으로 TLS로 명칭이 변경되었으나, 보통 이를 합쳐 SSL/TLS로 많이 부릅니다. 현재는 보안 취약점으로 인해 SSL과 TLS 1.0, 1.1은 사용이 중단되었고, **TLS 1.2와 TLS 1.3이 주로 사용됩니다.**

#### `OSI 7계층에서의 SSL/TLS`

SSL/TLS는 OSI 7계층에서 정확히 하나의 계층에 매핑되지 않고, **5계층(세션)과 6계층(표현)에 걸쳐 있습니다.**

```
┌─────────────────────────────────┐
│ 7. 응용 계층 (Application)       │ ← HTTP, FTP, SMTP 등
├─────────────────────────────────┤
│ 6. 표현 계층 (Presentation)      │ ← 암호화/복호화 담당
├─────────────────────────────────┤ ← SSL/TLS
│ 5. 세션 계층 (Session)           │ ← 세션 수립/관리 (핸드셰이크)
├─────────────────────────────────┤
...
SSL/TLS가 수행하는 기능이 두 계층의 역할을 모두 포함하기 때문입니다.

|      기능      |   OSI 계층   |             설명             |
| :------------: | :----------: | :--------------------------: |
| TLS 핸드셰이크 | 5계층 (세션) | 연결 수립, 세션 관리, 재연결 |
| 암호화/복호화  | 6계층 (표현) |  데이터 암호화, 인증서 검증  |

#### HTTPS가 제공하는 세 가지 보안 특성

HTTPS는 HTTP에 데이터 암호화가 추가된 프로토콜입니다. HTTP(80번 포트)와 다르게 443번 포트를 사용하며, 다음 세 가지 보안 특성을 제공합니다.

- **기밀성(Confidentiality)**: 데이터를 암호화하여 제3자가 내용을 볼 수 없도록 합니다.
- **무결성(Integrity)**: 데이터가 전송 중에 변조되지 않았음을 보장합니다.
- **인증(Authentication)**: 통신 상대방(서버)이 신뢰할 수 있는 대상인지 확인합니다.

### `암호화 키의 종류에 대해 아나요?`

HTTPS를 이해하려면 먼저 암호화 키의 종류를 알아야 합니다.

```
암호화 방식
├── 대칭키 암호화: 키 1개 (암호화 = 복호화에 같은 키 사용)
│
└── 비대칭키 암호화: 키 2개 (공개키 + 개인키)
    ├── 공개키: 누구나 알 수 있는 키
    └── 개인키: 나만 가지고 있는 키
```
...
| 비대칭키 | 암호화/복호화에 **다른 키** 사용 (공개키 + 개인키) |
|  공개키  | 비대칭키 중 **공개하는 키**                        |
|  개인키  | 비대칭키 중 **비밀로 유지하는 키**                 |

#### `비대칭키(공개키/개인키) 암호화`

**공개키**로 암호화하면 **개인키**로만 복호화할 수 있고, 그 반대도 가능합니다.
...
|   암호화 방식   |      복호화       |        용도        |
| :-------------: | :---------------: | :----------------: |
| 공개키로 암호화 | 개인키로만 복호화 |  기밀 데이터 전송  |
| 개인키로 암호화 | 공개키로만 복호화 | 서명 (신뢰성 보장) |

|       장점        |          단점          |
| :---------------: | :--------------------: |
| 키 전달 문제 해결 | 느림 (연산 비용 높음)  |
| 디지털 서명 가능  | 대용량 데이터에 부적합 |

#### `대칭키와 비대칭키의 혼합 사용`

실제 HTTPS 통신에서는 비대칭키 암호화만 사용하지 않습니다. **비대칭키는 연산 비용이 높아** 대량의 데이터를 암호화하기에 비효율적이기 때문입니다.

따라서 HTTPS는 두 방식을 조합합니다.
...
|    단계     |       사용하는 키        | 이유                 |
| :---------: | :----------------------: | -------------------- |
|   키 교환   | 비대칭키 (공개키/개인키) | 안전하게 대칭키 전달 |
| 데이터 통신 |     대칭키 (세션 키)     | 빠른 암호화/복호화   |

이렇게 하면 **비대칭키의 보안성**과 **대칭키의 효율성**을 모두 얻을 수 있습니다.

#### TLS 핸드셰이크 과정

TLS 핸드셰이크는 클라이언트와 서버가 암호화 통신을 시작하기 전에 서로를 인증하고, 세션 키를 교환하는 과정입니다.

> 서버의 **공개키**로 **대칭키(세션 키)**를 안전하게 교환하는 과정

#### TLS 1.3 핸드셰이크 (1-RTT)

| 단계 | 쉬운 설명                    | 기술 용어                                        |       사용하는 키        |
| :--: | ---------------------------- | ------------------------------------------------ | :----------------------: |
|  1   | "나 이런 자물쇠 열 수 있어"  | Client Hello (암호화 스위트 목록 + 키 공유 정보) |            -             |
|  2   | "이 자물쇠 쓰자 + 내 신분증" | Server Hello + Certificate                       |  서버의 **공개키** 전달  |
|  3   | "신분증 진짜인지 확인"       | CA 인증서 검증                                   |  CA의 **공개키**로 검증  |
|  4   | "비밀번호를 암호화해서 보냄" | 키 교환                                          | 서버 **공개키**로 암호화 |
|  5   | "둘 다 같은 비밀번호 보유"   | 세션 키 생성                                     |   **대칭키** 생성 완료   |
|  6   | "금고로 데이터 주고받기"     | 암호화 통신 시작                                 |    **대칭키**로 통신     |

#### TLS 1.2와의 차이점

|      항목       |       TLS 1.2        |           TLS 1.3            |
| :-------------: | :------------------: | :--------------------------: |
| 핸드셰이크 속도 |        2-RTT         |            1-RTT             |
|     재연결      |          -           |          0-RTT 지원          |
|      보안       | 취약한 알고리즘 포함 | RSA 키 교환, RC4, SHA-1 제거 |
|   암호화 시점   |  핸드셰이크 완료 후  |    인증서 전송부터 암호화    |

### HTTPS 인증서 발급 과정에 대해 아나요?

HTTPS를 사용하기 위해서는 인증 기관 CA(Certificate Authority)에서 인증서를 발급받아야 합니다.
...
| 단계 | 설명                                 |          사용하는 키          |
| :--: | ------------------------------------ | :---------------------------: |
|  1   | 서버가 공개키와 정보를 CA에 제출     |        서버 **공개키**        |
|  2   | CA가 서버 정보를 검증 후 인증서 발급 |     CA **개인키**로 서명      |
|  3   | 서버가 브라우저에 인증서 전달        | 인증서에 서버 **공개키** 포함 |
|  4   | 브라우저가 인증서 검증               |   CA **공개키**로 서명 확인   |

브라우저에는 신뢰할 수 있는 **CA의 공개키 목록**이 미리 내장되어 있어, 해당 CA에서 발급한 인증서만 경고 없이 신뢰합니다.

## 실시간 통신

📌 **관련 주제**: [HTTP](#http), [네트워크](#네트워크), [REST API (js.md)](./js.md#rest-api)

### SSE(Server-Sent Events)가 무엇인가요?

SSE는 **HTTP 기반의 단방향(서버→클라이언트) 스트리밍** 기술입니다. 서버가 클라이언트에게 `text/event-stream` MIME 타입으로 데이터를 지속적으로 푸시하며, 별도의 프로토콜 없이 기존 HTTP 위에서 동작합니다. 연결이 끊어지면 브라우저의 `EventSource` API가 **자동으로 재연결**을 시도하므로 안정성이 높습니다.

<details>
<summary>상세 설명 및 예제 보기</summary>

#### EventSource API 사용 예제
...
event: update
data: {"message": "새 알림입니다"}

data: 기본 메시지 (event 생략 시 onmessage로 수신)
```

#### 활용 사례

- 실시간 알림 (SNS 알림, 주문 상태 등)
- 실시간 피드 업데이트 (뉴스, 주식 시세)
- 서버 로그 스트리밍
- AI 응답 스트리밍 (ChatGPT 등)

</details>

### SSE를 사용할 때 메시지의 순서가 보장되나요?

네, SSE는 **TCP 위의 HTTP 연결**을 사용하므로 메시지 순서가 보장됩니다. 또한 각 이벤트에 `id` 필드를 포함하면, 연결이 끊어졌다가 재연결될 때 브라우저가 `Last-Event-ID` 헤더로 마지막 수신 ID를 서버에 전달합니다. 서버는 이를 기반으로 **누락된 이벤트부터 재전송**할 수 있어 메시지 유실도 방지할 수 있습니다.
...
app.get('/events', (req, res) => {
  ...
  // lastId 이후의 이벤트부터 전송
  getEventsSince(lastId).forEach((event) => {
...
SSE를 클라이언트에서 소비하는 방식은 두 가지입니다. **EventSource API**는 브라우저 내장 인터페이스로 `text/event-stream` 파싱과 자동 재연결을 제공하지만, **GET 요청만** 가능하고 커스텀 헤더를 설정할 수 없습니다. 반면 **Fetch + ReadableStream** 방식은 `response.body.getReader()`로 스트림을 직접 읽어 파싱하며, POST 요청이나 인증 헤더 등 **자유로운 요청 설정**이 가능합니다.
...
|      항목       |              EventSource               | Fetch + ReadableStream  |
| :-------------: | :------------------------------------: | :---------------------: |
| **HTTP 메서드** |               GET만 가능               |    GET, POST 등 자유    |
| **커스텀 헤더** |                  불가                  | 가능 (Authorization 등) |
| **자동 재연결** |             브라우저 내장              |     직접 구현 필요      |
| **이벤트 파싱** | 자동 (`onmessage`, `addEventListener`) |     직접 파싱 필요      |
|  **CORS 제어**  |                 제한적                 |    세밀한 제어 가능     |

#### Fetch + ReadableStream 사용 예제
...
- **EventSource**: 실시간 알림, 뉴스 피드 등 단순 GET 구독
- **Fetch + ReadableStream**: AI 챗봇 응답 스트리밍 (POST로 프롬프트 전송 필요), 인증이 필요한 SSE 엔드포인트

</details>

### WebSocket이 무엇인가요?

WebSocket은 HTTP 업그레이드 요청을 통해 연결을 수립한 뒤, **양방향 전이중(full-duplex) 통신**을 제공하는 프로토콜입니다. `ws://` 또는 `wss://`(TLS) 스킴을 사용하며, 한 번 연결되면 클라이언트와 서버가 언제든 자유롭게 메시지를 주고받을 수 있습니다. HTTP의 요청-응답 패턴과 달리 **지속적인 연결**을 유지하므로 헤더 오버헤드가 적고 지연이 낮습니다.
...
SSE는 **HTTP 기반 단방향** 통신으로 서버에서 클라이언트로만 데이터를 보내며, WebSocket은 **전용 프로토콜 기반 양방향** 통신으로 양쪽 모두 자유롭게 메시지를 주고받습니다. SSE는 자동 재연결과 텍스트 전송에 강점이 있고, WebSocket은 바이너리 데이터와 저지연 양방향 통신에 적합합니다.

|         항목          |           SSE            |        WebSocket        |
| :-------------------: | :----------------------: | :---------------------: |
|     **통신 방향**     | 단방향 (서버→클라이언트) |  양방향 (전이중 통신)   |
|     **프로토콜**      |           HTTP           |     ws:// / wss://      |
|    **자동 재연결**    |    브라우저 내장 지원    |     직접 구현 필요      |
|    **데이터 형식**    |      텍스트만 가능       |    텍스트 + 바이너리    |
| **HTTP/2 멀티플렉싱** |           지원           |         미지원          |
|    **적합한 사례**    | 알림, 피드, AI 스트리밍  | 채팅, 게임, 실시간 협업 |

### Polling과 Long Polling에 대해 설명해주세요

**Polling**은 클라이언트가 일정 주기로 서버에 HTTP 요청을 보내 새 데이터가 있는지 확인하는 방식입니다. **Long Polling**은 서버가 새 데이터가 생길 때까지 응답을 보류했다가, 데이터가 준비되면 응답을 보내는 방식으로 Polling의 불필요한 요청을 줄인 개선 형태입니다. 두 방식 모두 기존 HTTP 인프라에서 동작하지만, 실시간성과 효율은 SSE나 WebSocket에 비해 떨어집니다.

<details>
<summary>상세 설명 및 발전 흐름 보기</summary>

#### Polling vs Long Polling 비교

```
[Polling]
클라이언트 ──요청──▶ 서버 (데이터 없음) ──즉시 빈 응답──▶ 클라이언트
클라이언트 ──요청──▶ 서버 (데이터 없음) ──즉시 빈 응답──▶ 클라이언트
클라이언트 ──요청──▶ 서버 (데이터 있음) ──데이터 응답──▶ 클라이언트

[Long Polling]
클라이언트 ──요청──▶ 서버 (대기...) ──데이터 발생 시 응답──▶ 클라이언트
클라이언트 ──즉시 재요청──▶ 서버 (대기...) ──데이터 발생 시 응답──▶ 클라이언트
```

#### 실시간 통신 기술의 발전 흐름

| 기술         | 방향   | 실시간성 | 서버 부하 | 구현 난이도 |
| :----------- | :----- | :------: | :-------: | :---------: |
| Polling      | 단방향 |    낮    |    높     |    쉬움     |
| Long Polling | 단방향 |    중    |    중     |    보통     |
| SSE          | 단방향 |    높    |    낮     |    쉬움     |
| WebSocket    | 양방향 |    높    |    낮     |    보통     |

**선택 기준**: 단순한 서버→클라이언트 푸시라면 SSE, 양방향 실시간 통신이 필요하면 WebSocket, 레거시 환경 호환이 중요하면 Long Polling을 고려합니다.
