...
📌 **관련 주제**: [웹 표준 및 웹 접근성](#웹-표준-및-웹-접근성), [CSS](#css)

## DOCTYPE

📌 **관련 주제**: [웹 표준 및 웹 접근성](#웹-표준-및-웹-접근성), [그 외](#그-외)
...
meta 태그의 주요 요소는 **charset(문자 집합), name(메타 정보 형태), content(실제 메타 데이터)** 입니다. **charset="utf-8"은 전세계 언어의 문자를 포함하는 문자 집합**을 의미하고, **name 속성은 author, description, keyword, viewport 등의 정보 형태**를 지정하며, **content 속성은 실제 메타 데이터 내용**을 담습니다. 검색 엔진은 name="description"의 content 내용을 검색 결과에 표시합니다.

<details>
<summary>meta 태그 요소 상세 설명 보기</summary>
...
📌 **관련 주제**: [DOCTYPE](#doctype), [그 외](#그-외), [CSS](#css)
...
- `canvas` 기능 추가
  ...
  - 픽셀 기반의 비트맵 이미지를 그리는 기능을 제공하며, 그래프, 게임 그래픽, 기타 실시간 이미지를 그릴 수 있다.
...
HTML 텍스트 관련 태그는 **줄바꿈이 일어나는 태그(h1~h6, hr, pre, blockquote)** 와 **줄바꿈이 일어나지 않는 태그(strong, b, em, i, q, mark)** 로 구분됩니다. **제목 태그(h1~h6)는 순차적으로 사용**해야 하며, **strong과 em은 의미를 가진 강조**, **b와 i는 단순 스타일**을 의미합니다. 접근성을 위해 스크린 리더가 제목 구조를 파악할 수 있도록 제목 단계를 건너뛰지 않아야 합니다.

<details>
<summary>텍스트 관련 태그 상세 설명 보기</summary>
...
📌 **관련 주제**: [DOCTYPE](#doctype), [웹 표준 및 웹 접근성](#웹-표준-및-웹-접근성), [CSS](#css)
...
📌 **관련 주제**: [HTML](#html), [웹 표준 및 웹 접근성](#웹-표준-및-웹-접근성)
...
grid를 사용하면 list에 width를 따로 입력하지 않고 리스트에 간격과 width 비율만 입력해주면 쉽고 간편히 만들 수 있습니다.
