...
- [리액트에서 리렌더링이 발생하는 조건은 무엇인가요 🔥🔥](#리액트에서-리렌더링이-발생하는-조건은-무엇인가요)
...
**리액트를 사용하는 주요 이유는 컴포넌트 기반 구조, Virtual DOM을 통한 효율적인 렌더링, 그리고 풍부한 생태계입니다.** 컴포넌트를 통해 UI를 독립적으로 관리하고 재사용할 수 있으며, Virtual DOM 덕분에 최소한의 DOM 조작으로 빠른 렌더링이 가능합니다. 또한 방대한 생태계와 커뮤니티 지원, React Native를 통한 크로스 플랫폼 개발도 큰 장점입니다.

<details>
<summary>리액트 사용 이유 상세 설명 보기</summary>
...
**리액트 컴포넌트는 state 변경, props 변경, 부모 컴포넌트 리렌더링, Context 값 변경 시 리렌더링됩니다.** 클래스 컴포넌트에서는 `forceUpdate()`로 강제 리렌더링도 가능합니다. 리액트는 상태가 변경되면 해당 컴포넌트부터 하위 트리 전체를 리렌더링하며, `React.memo`, `useMemo`, `useCallback` 등으로 불필요한 리렌더링을 방지할 수 있습니다.

<details>
<summary>리렌더링 조건 상세 설명 보기</summary>

#### 1. state가 변경될 때

`setState`(클래스)나 `useState`의 setter 함수(함수 컴포넌트)로 상태를 변경하면 해당 컴포넌트가 리렌더링됩니다. 리액트는 `Object.is`로 이전 state와 비교하여, 같은 값이면 리렌더링을 건너뜁니다.

```jsx
const [count, setCount] = useState(0)
setCount(1) // state가 변경되어 리렌더링 발생
setCount(0) // 이전 값과 같으므로 리렌더링 발생하지 않음
```

#### 2. props가 변경될 때

부모로부터 전달받은 props 값이 변경되면 자식 컴포넌트가 리렌더링됩니다. 단, 부모가 리렌더링되면 props가 실제로 변경되지 않아도 자식은 기본적으로 리렌더링됩니다. 이를 방지하려면 `React.memo`를 사용합니다.

#### 3. 부모 컴포넌트가 리렌더링될 때

부모 컴포넌트가 리렌더링되면 **모든 자식 컴포넌트도 리렌더링**됩니다. 이것이 불필요한 리렌더링의 가장 흔한 원인입니다.
...
`useContext`로 구독 중인 Context의 값이 변경되면, 해당 Context를 사용하는 모든 컴포넌트가 리렌더링됩니다. 중간 컴포넌트가 `React.memo`로 감싸져 있어도 Context를 직접 사용하는 컴포넌트는 리렌더링을 피할 수 없습니다.

#### 5. forceUpdate 호출 (클래스 컴포넌트)

클래스 컴포넌트에서 `this.forceUpdate()`를 호출하면 `shouldComponentUpdate`를 건너뛰고 강제로 리렌더링합니다.

#### 리렌더링이 발생하지 않는 경우

- `useRef`의 `.current` 값 변경 — ref는 리액트 생명주기에 독립적
- DOM을 직접 조작한 경우 — 리액트가 추적하지 않음
- `setState`에 이전과 동일한 값을 전달한 경우

#### 불필요한 리렌더링 방지 방법

| 방법          | 용도                                                    |
| ------------- | ------------------------------------------------------- |
| `React.memo`  | props가 변경되지 않은 컴포넌트의 리렌더링 방지          |
| `useMemo`     | 계산 비용이 높은 값의 재계산 방지                       |
| `useCallback` | 함수 참조의 불필요한 변경 방지                          |
| state 분리    | 관련 없는 state를 별도 컴포넌트로 분리                  |
| Context 분리  | 자주 변경되는 값과 그렇지 않은 값을 별도 Context로 분리 |
...
리덕스는 FLUX 패턴을 기반으로 한 상태관리 라이브러리로, Store(스토어)라는 단일 저장소에서 전역 상태를 관리합니다. props drilling 없이 어디서든 상태에 접근할 수 있으며, **단일 스토어**, **읽기 전용 상태**, **순수 함수(리듀서)에 의한 변경**이라는 3가지 원칙을 따릅니다. 현재는 Redux Toolkit(RTK)이 표준이며, 보일러플레이트를 크게 줄여줍니다.

<details>
<summary>리덕스 3가지 원칙 상세</summary>

1. **단일 스토어(Single source of truth)** — 애플리케이션의 전역 상태는 하나의 스토어 내 트리에 저장됩니다.
2. **읽기 전용 상태(State is read-only)** — 상태를 직접 변경할 수 없고, 액션을 디스패치하여 변경합니다.
3. **순수 함수에 의한 변경(Changes are made with pure functions)** — 리듀서는 이전 상태와 액션을 받아 새로운 상태를 반환하는 순수 함수여야 합니다.

</details>

### 현대 상태 관리 라이브러리에는 어떤 것들이 있나요?

현대 프론트엔드에서는 상태를 **클라이언트 상태**(UI, 폼 등)와 **서버 상태**(API 데이터, 캐시)로 구분하여 관리하는 추세입니다. 클라이언트 상태에는 **Zustand**(간결한 Flux 기반), **Jotai**(원자적 상태)가, 서버 상태에는 **TanStack Query**(자동 캐싱·리페칭)가 대표적입니다. Redux Toolkit도 여전히 대규모 프로젝트에서 사용되지만, 새 프로젝트에서는 더 가벼운 대안이 선호됩니다.

|                    | **Zustand**            | **Jotai**               | **TanStack Query**      | **Redux Toolkit**           |
| ------------------ | ---------------------- | ----------------------- | ----------------------- | --------------------------- |
| **분류**           | 클라이언트 상태        | 클라이언트 상태         | 서버 상태               | 클라이언트 상태             |
| **핵심 특징**      | Flux 기반, 단일 스토어 | 원자(atom) 단위 상태    | 자동 캐싱·리페칭·동기화 | Flux 기반, 미들웨어 생태계  |
| **보일러플레이트** | 매우 적음              | 매우 적음               | 적음 (훅 기반)          | 중간 (슬라이스 패턴)        |
| **적합한 사례**    | 중소규모 앱 전역 상태  | 독립적인 상태가 많은 앱 | API 데이터 중심 앱      | 복잡한 상태 로직, 대규모 앱 |

<details>
<summary>각 라이브러리 코드 예제</summary>

**Zustand** — 스토어 생성과 사용이 매우 간결합니다.
...
**Jotai** — 원자(atom) 단위로 상태를 정의하고, 필요한 컴포넌트에서만 구독합니다.
...
**TanStack Query** — 서버 데이터 페칭, 캐싱, 동기화를 자동 처리합니다.
...
**라이프사이클 메서드는 클래스 컴포넌트에서 컴포넌트의 생명주기(마운트, 업데이트, 언마운트)마다 특정 작업을 수행할 수 있게 하는 메서드입니다.** will 접두사는 작업 전, Did 접두사는 작업 후에 실행됩니다. 주요 메서드로는 componentDidMount(마운트 후), componentDidUpdate(업데이트 후), componentWillUnmount(언마운트 전)가 있으며, 함수 컴포넌트에서는 useEffect 훅으로 동일한 기능을 구현할 수 있습니다.

<details>
<summary>라이프사이클 메서드 상세 설명 보기</summary>

<br/>

라이프사이클 메서드의 종류는 총 아홉 가지입니다. 이 메서드들은 컴포넌트 클래스에서 덮어 써 선언함으로써 사용할 수 있습니다. 라이프사이클은 총 세 가지, 즉 마운트, 업데이트, 언마운트 카테고리로 나눕니다.
...
**Hooks는 리액트 16.8에 도입된 기능으로, 함수 컴포넌트에서 상태 관리와 생명주기 기능을 사용할 수 있게 합니다.** 주요 Hooks로는 useState(상태 관리), useEffect(부수효과 처리), useReducer(복잡한 상태 로직), useMemo(값 메모이제이션), useCallback(함수 메모이제이션), useRef(DOM 접근 및 값 유지)가 있으며, 커스텀 Hook을 만들어 로직을 재사용할 수 있습니다.

<details>
<summary>각 Hook의 상세 설명 및 예제 보기</summary>
...
리액트 컴포넌트가 렌더링될 때마다 특정 작업을 수행하도록 설정할 수 있는 Hook입니다. 클래스형 컴포넌트의 componentDidMount와 componentDidUpdate를 합친 형태로 보아도 무방합니다.

**마운트만 실행시키고 싶을 경우**: 두 번째 파라미터로 비어있는 배열을 넣어주면 됩니다.
...
useMemo와 상당히 비슷한 함수입니다. 주로 렌더링 성능을 최적화 해야하는 상황에서 사용합니다. 이 Hook을 사용하면 **만들어놨던 함수를 재사용할 수 있습니다.** useCallback의 첫 번째 파라미터에는 생성하고 싶은 함수를 넣고, 두 번째 파라미터에는 배열을 넣으면 됩니다. 이 배열에는 어떤 값이 바뀌었을 때 함수를 새로 생성해야 하는지 명시해야 합니다.
...
커스텀 Hook은 개발자가 직접 만드는 재사용 가능한 Hook입니다. 여러 컴포넌트에서 반복되는 로직을 추출하여 독립적인 함수로 만들 수 있으며, use로 시작하는 이름으로 만들면 됩니다. 커스텀 Hook 안에서 다른 Hook들(useState, useEffect 등)을 자유롭게 사용할 수 있습니다.
